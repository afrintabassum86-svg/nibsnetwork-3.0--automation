...
async function exchangeToFile() {
    try {
        ...
        if (data.error) {
            fs.writeFileSync('exchange_token.txt', 'ERROR: ' + JSON.stringify(data.error));
            return;
        }

        fs.writeFileSync('exchange_token.txt', data.access_token);
        console.log('Token written to exchange_token.txt');
    } catch (e) {
